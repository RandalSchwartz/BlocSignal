...
/// Cubit managing selected install package tab in the Hero terminal.
class HeroTabCubit() extends CubitSignal<String> {
  this : super(initialState: 'Flutter');

  void selectTab(String tab) => emit(tab);
}

class const HeroBanner({super.key}) extends StatelessComponent {
  @override
  Component build(BuildContext context) {
    return BlocSignalProvider<HeroTabCubit>(
      create: (_) => HeroTabCubit(),
      child: const _HeroBannerContent(),
    );
  }
}

class const _HeroBannerContent() extends StatefulComponent {
  @override
  State<_HeroBannerContent> createState() => _HeroBannerContentState();
}

class _HeroBannerContentState() extends State<_HeroBannerContent> {
  ...
  void _copyCommand(String activeCommand) {
    try {
      web.window.navigator.clipboard.writeText(activeCommand);
    } catch (_) {
  ...
  @override
  Component build(BuildContext context) {
    return BlocSignalBuilder<HeroTabCubit, String>(
      builder: (context, selectedTab) {
        final activeCommand =
            _installCommands[selectedTab] ?? 'dart pub add bloc_signals';
        ...
        return section(classes: 'hero-section', [
          div(classes: 'container hero-container', [
            ...
            // Interactive Tabbed Install Terminal powered by BlocSignalBuilder
            div(classes: 'install-terminal-box', [
              div(classes: 'install-terminal-header', [
                div(classes: 'install-tabs-list', [
                  for (final tab in _installCommands.keys)
                    button(
                      classes:
                          'install-tab-btn ${selectedTab == tab ? "active" : ""}',
                      onClick: () =>
                          context.read<HeroTabCubit>().selectTab(tab),
                      [Component.text(tab)],
                    ),
                ]),
                button(
                  classes: 'btn-copy-install ${_isCopied ? "copied" : ""}',
                  onClick: () => _copyCommand(activeCommand),
              ...
              div(classes: 'install-terminal-body', [
                ...
                code(classes: 'terminal-command', [
                  Component.text(activeCommand),
                ]),
              ]),
            ]),
                ...
                div(classes: 'comparison-card comparison-card-classic', [
                  div(classes: 'comp-card-header', [
                    div(classes: 'comp-header-left', [
                      ...
                      h4(classes: 'comp-title', [
                        Component.text('Classic BLoC'),
                      ]),
                    ]),
