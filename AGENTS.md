...
5. **Lifecycle & Disposal**: `close()` marks `isClosed = true` and cleans up effects. Subsequent `add()` or `emit()` calls assert in debug mode (`assert(!isClosed)`) and are safely dropped in release mode.
